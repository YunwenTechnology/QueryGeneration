...
parser.add_argument(
    '-base_fn',
    dest='base_fn',
    default='/data/lcong/gpt2/data/doc2query_',
    type=str,
...
parser.add_argument(
    '-input_fn',
    dest='input_fn',
    default='/data/lcong/gpt2/data/doc2query.json',
    type=str,
    ...
    with open(args.input_fn, "r") as f:
        for l_no, l in enumerate(f):
            if l_no % args.num_folds == args.fold:
                ...
                tokens = []
                query = tokenization.convert_to_unicode(article["query"])
                query_tokens = tokenizer.tokenize(query)
                c_size = final_desired_size - len(query_tokens) - 2
                content = tokenization.convert_to_unicode(article["content"])
                content_tokens = tokenizer.tokenize(content)
                if len(content_tokens) > c_size:
                    content_tokens = content_tokens[:c_size]
...
def buffered_and_sliding_window_article_iterator(tokenizer, final_desired_size=1025):
    ...
    for article in article_iterator(tokenizer, final_desired_size):
...
total_written = 0
train_file = args.base_fn + 'train_mrc_{:04d}.tfrecord'.format(args.fold)
with S3TFRecordWriter(train_file) as train_writer:
    for article in buffered_and_sliding_window_article_iterator(tokenizer,final_desired_size=args.max_seq_length + 1):
